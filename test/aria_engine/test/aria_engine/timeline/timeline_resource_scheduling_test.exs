...
defmodule Timeline.ResourceSchedulingTest do
  ...
  describe "capability-dependent activity assignment" do
    ...
    test "assigns specialized manufacturing tasks correctly" do
      ...
      welder = AgentEntity.create_agent(
        "welder1",
        "Certified Welder",
      ...
      assembler = AgentEntity.create_agent(
        "assembler1",
        "Assembly Tech",
        %{experience: "5 years"},
      ...
      welding_station = AgentEntity.create_entity(
        "weld_station_1",
        "Welding Station #1",
  ...
  describe "resource conflicts with capability requirements" do
    test "detects pilot scheduling conflict" do
      ...
      sequential_timeline = Timeline.add_constraint(
        updated_timeline,
        "#{flight1.id}_end",
        "#{flight2.id}_start",
        {0, :infinity}
    ...
    test "handles shared equipment with different operators" do
      ...
      |> Timeline.add_constraint("#{morning_shift.id}_end", "#{afternoon_shift.id}_start", {3600, 3600})
  ...
  describe "multi-agent coordination" do
    ...
    test "handles emergency response with multiple specialists" do
      ...
      fire_chief = AgentEntity.create_agent(
        "chief1",
        "Fire Chief",
        %{rank: "chief"},
      ...
      paramedic = AgentEntity.create_agent(
        "medic1",
        "Paramedic",
        %{certification: "EMT-P"},
      ...
      firefighter = AgentEntity.create_agent(
        "ff1",
        "Firefighter",
        %{specialty: "rescue"},
      ...
      medical_treatment = Interval.new(
        DateTime.from_naive!(~N[2025-01-01 15:00:00], "Etc/UTC"),
      ...
      |> Timeline.add_constraint("#{command_setup.id}_end", "#{rescue_operation.id}_start", {0, 0})
      |> Timeline.add_constraint("#{rescue_operation.id}_end", "#{medical_treatment.id}_start", {0, 0})
