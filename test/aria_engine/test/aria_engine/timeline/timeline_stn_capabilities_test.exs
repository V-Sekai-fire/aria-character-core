...
defmodule Timeline.STNCapabilitiesTest do
  ...
  describe "PC-2 algorithm with capability constraints" do
    test "propagates constraints through capability-dependent chain" do
      ...
      |> Timeline.add_constraint("#{engineering_phase.id}_end", "#{construction_phase.id}_start", {57600, 57600}) # 16 hour gap
  ...
  describe "constraint solving with agent capabilities" do
    test "solves timeline with capability-dependent scheduling" do
      ...
      |> Timeline.add_constraint("#{surgery_phase.id}_end", "#{recovery_phase.id}_start", {0, 0})
    ...
    test "handles constraint propagation with capability transitions" do
      ...
      auto_robot = AgentEntity.add_capabilities(manual_robot, [:autonomous_operation, :decision_making])
      auto_phase = Interval.new(
        DateTime.from_naive!(~N[2025-01-01 12:00:00], "Etc/UTC"),
        DateTime.from_naive!(~N[2025-01-01 16:00:00], "Etc/UTC"),
        agent: auto_robot,
        label: "Autonomous Operation",
      ...
      |> Timeline.add_constraint("#{offline_phase.id}_end", "#{manual_phase.id}_start", {0, 0}) # immediate transition
      |> Timeline.add_constraint("#{manual_phase.id}_end", "#{auto_phase.id}_start", {0, 0}) # immediate transition
  ...
  describe "temporal network consistency with capabilities" do
    test "validates complex capability-dependent network" do
      ...
      |> Timeline.add_constraint("#{planning.id}_end", "#{test_planning.id}_start", {7200, 7200}) # 2 hour delay
    ...
    test "detects inconsistencies in capability-constrained networks" do
      ...
      specialist = AgentEntity.create_agent(
        "spec1", "Specialist",
      ...
      constrained_timeline = Timeline.add_constraint(
        ...
        "#{task2.id}_start",
        "#{task1.id}_end",
        {-7200, -3600} # task2 must start 1-2 hours before task1 ends
      )
