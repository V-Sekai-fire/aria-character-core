...
defmodule Timeline.ScenariosTest do
  ...
  describe "autonomous vehicle fleet coordination" do
    test "coordinates multiple autonomous vehicles with dynamic capabilities" do
      ...
      delivery_van = AgentEntity.create_agent(
        "van1",
        "Delivery Van Alpha",
      ...
      passenger_car = AgentEntity.create_agent(
        "car1",
        "Passenger Car Beta",
      ...
      maintenance_vehicle = AgentEntity.create_entity(
        "maint1",
        "Maintenance Vehicle",
      ...
      passenger_service = Interval.new(
      ...
      charging_van = AgentEntity.remove_capabilities(delivery_van, [:autonomous_driving, :route_optimization])
      charging_period = Interval.new(
        DateTime.from_naive!(~N[2025-01-01 11:00:00], "Etc/UTC"),
      ...
      active_maintenance = AgentEntity.add_capabilities(maintenance_vehicle, [:diagnostic_analysis, :repair_operations])
      maintenance_service = Interval.new(
      ...
      |> Timeline.add_constraint("#{delivery_route.id}_end", "#{charging_period.id}_start", {0, 0})
      |> Timeline.add_constraint("#{charging_period.id}_start", "#{maintenance_service.id}_start", {1800, 1800}) # 30 min delay
    ...
    test "handles emergency response with capability reallocation" do
      ...
      ambulance = AgentEntity.create_agent(
        "amb1",
        "Emergency Ambulance",
      ...
      patrol_car = AgentEntity.create_agent(
        "patrol1",
        "Police Patrol",
      ...
      emergency_response = Interval.new(
        DateTime.from_naive!(~N[2025-01-01 12:30:00], "Etc/UTC"),
      ...
      |> Timeline.add_constraint("#{emergency_response.id}_start", "#{traffic_support.id}_start", {0, 0}) # simultaneous response
      |> Timeline.add_constraint("#{emergency_response.id}_end", "#{traffic_support.id}_end", {0, 0}) # simultaneous end
  ...
  describe "smart city infrastructure coordination" do
    test "coordinates traffic lights and autonomous vehicles" do
      ...
      traffic_light_a = AgentEntity.create_entity(
        "light_a",
        "Traffic Light A",
        %{intersection: "main_and_first", sensors: ["camera", "radar"]}
      )
      
      traffic_controller = AgentEntity.create_agent(
        "controller1",
      ...
      autonomous_bus = AgentEntity.create_agent(
        "bus1",
      ...
      smart_light = AgentEntity.add_capabilities(traffic_light_a, [:adaptive_timing, :vehicle_communication])
      
      # Morning rush hour optimization
      traffic_optimization = Interval.new(
      ...
      |> Timeline.add_constraint("#{traffic_optimization.id}_start", "#{adaptive_timing.id}_start", {1800, 1800}) # 30 min delay
      |> Timeline.add_constraint("#{adaptive_timing.id}_start", "#{priority_route.id}_start", {1800, 1800}) # 30 min delay
    ...
    test "handles power grid and electric vehicle coordination" do
      ...
      power_station = AgentEntity.create_agent(
        "station1",
        "Smart Power Station",
      ...
      charging_station = AgentEntity.create_entity(
        "charger1",
        "Fast Charging Station",
        %{power_rating: "150kW", connectors: 4}
      )
      
      electric_vehicle = AgentEntity.create_agent(
        "ev1",
      ...
      smart_charger = AgentEntity.add_capabilities(charging_station, [:dynamic_pricing, :load_management])
      smart_charging = Interval.new(
      ...
      |> Timeline.add_constraint("#{load_balancing.id}_start", "#{smart_charging.id}_start", {3600, 3600}) # 1 hour delay
      |> Timeline.add_constraint("#{smart_charging.id}_start", "#{vehicle_charging.id}_start", {1800, 1800}) # 30 min delay
