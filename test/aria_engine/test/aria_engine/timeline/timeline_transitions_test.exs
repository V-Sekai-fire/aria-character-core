...
defmodule Timeline.TransitionsTest do
  ...
  describe "agent to entity transitions" do
    ...
    test "agent transitions to entity when all action capabilities removed" do
      ...
      worker = AgentEntity.create_agent(
        "worker1",
        "Factory Worker",
        %{shift: "day"},
      ...
      inactive_worker = AgentEntity.remove_capabilities(worker, [:assembly, :quality_check, :decision_making])
  ...
  describe "dynamic capability changes during timeline" do
    test "handles multiple transitions in single timeline" do
      ...
      auto_machine = AgentEntity.add_capabilities(manual_machine, [:autonomous_operation, :decision_making])
      auto_interval = Interval.new(
        DateTime.from_naive!(~N[2025-01-01 12:00:00], "Etc/UTC"),
      ...
      maintenance_machine = AgentEntity.remove_capabilities(auto_machine, [:manual_operation, :autonomous_operation, :decision_making])
      maintenance_interval = Interval.new(
        DateTime.from_naive!(~N[2025-01-01 16:00:00], "Etc/UTC"),
    ...
    test "validates transition consistency" do
      # Create agent
      drone = AgentEntity.create_agent(
        "drone1",
        "Delivery Drone",
        %{battery: 100},
      ...
      # Remove all action capabilities
      inactive_drone = AgentEntity.remove_capabilities(grounded_drone, [:navigation, :package_delivery])
