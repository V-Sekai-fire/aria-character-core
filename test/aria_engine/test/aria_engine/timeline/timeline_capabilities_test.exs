...
defmodule Timeline.CapabilitiesTest do
  ...
  describe "capability-based agent/entity transitions" do
    test "entity becomes agent when given action capabilities" do
      ...
      driving_interval = Interval.new(
        DateTime.from_naive!(~N[2025-01-01 12:00:00], "Etc/UTC"),
        DateTime.from_naive!(~N[2025-01-01 14:00:00], "Etc/UTC"),
        agent: autonomous_car, label: "Autonomous Driving"
      )
    ...
    test "agent becomes entity when capabilities are removed" do
      ...
      # Start with an agent
      robot = AgentEntity.create_agent(
        "robot1", 
        "Industrial Robot",
        %{model: "ABB IRB 6700"},
      ...
      work_interval = Interval.new(
        DateTime.from_naive!(~N[2025-01-01 10:00:00], "Etc/UTC"),
        DateTime.from_naive!(~N[2025-01-01 12:00:00], "Etc/UTC"),
        agent: robot, label: "Active Welding"
      )
      ...
      inactive_robot = AgentEntity.remove_capabilities(robot, [:welding, :decision_making, :movement])
      refute AgentEntity.is_currently_agent?(inactive_robot)
      
      # Create interval as entity (maintenance mode)
      maintenance_interval = Interval.new(
        DateTime.from_naive!(~N[2025-01-01 12:00:00], "Etc/UTC"),
        DateTime.from_naive!(~N[2025-01-01 14:00:00], "Etc/UTC"),
        entity: inactive_robot, label: "Maintenance Mode"
      )
    ...
    test "dynamic capability changes during timeline execution" do
      ...
      basic_interval = Interval.new(
        DateTime.from_naive!(~N[2025-01-01 08:00:00], "Etc/UTC"),
        DateTime.from_naive!(~N[2025-01-01 10:00:00], "Etc/UTC"),
        entity: device, label: "Basic Operation"
      )
      
      # Phase 2: Gains communication capability
      comm_device = AgentEntity.add_capabilities(device, [:communication])
      comm_interval = Interval.new(
        DateTime.from_naive!(~N[2025-01-01 10:00:00], "Etc/UTC"),
        DateTime.from_naive!(~N[2025-01-01 12:00:00], "Etc/UTC"),
        agent: comm_device, label: "Communication Enabled"
      )
      
      # Phase 3: Gains full autonomy
      autonomous_device = AgentEntity.add_capabilities(comm_device, [:decision_making, :autonomous_operation])
      autonomous_interval = Interval.new(
        DateTime.from_naive!(~N[2025-01-01 12:00:00], "Etc/UTC"),
        DateTime.from_naive!(~N[2025-01-01 14:00:00], "Etc/UTC"),
        agent: autonomous_device, label: "Fully Autonomous"
      )
  ...
  describe "resource constraint testing with capabilities" do
    test "capability-dependent activity assignment" do
      ...
      surgeon = AgentEntity.create_agent(
        "surgeon1",
        "Dr. Smith",
        %{specialty: "cardiac"},
      ...
      nurse = AgentEntity.create_agent(
        "nurse1", 
        "Nurse Johnson",
        %{certification: "RN"},
    ...
    test "resource conflicts with capability requirements" do
      ...
      pilot = AgentEntity.create_agent(
        "pilot1",
        "Captain Smith", 
        %{license: "commercial"},
      ...
      timeline = Timeline.add_constraint(
        timeline,
        "#{flight1.id}_end",
        "#{flight2.id}_start", 
        {0, :infinity}  # flight2 must start after flight1 ends
      )
    ...
    test "multi-agent capability coordination" do
      ...
      project_manager = AgentEntity.create_agent(
        "pm1",
        "Alice Manager",
        %{experience: "10 years"},
      ...
      developer = AgentEntity.create_agent(
        "dev1", 
        "Bob Developer",
        %{language: "Elixir"},
      ...
      tester = AgentEntity.create_agent(
        "qa1",
        "Carol Tester", 
      ...
      development_phase = Interval.new(
        DateTime.from_naive!(~N[2025-01-01 10:00:00], "Etc/UTC"),
        DateTime.from_naive!(~N[2025-01-01 14:00:00], "Etc/UTC"),
        agent: developer,
        label: "Development",
      ...
      |> Timeline.add_constraint("#{planning_phase.id}_end", "#{development_phase.id}_start", {0, 0})
      |> Timeline.add_constraint("#{development_phase.id}_end", "#{testing_phase.id}_start", {0, 0})
  ...
  describe "entity ownership and temporal constraints" do
    test "entity ownership transfers during timeline" do
      ...
      conference_room = AgentEntity.create_entity(
        "room1",
        "Conference Room A",
    ...
    test "shared resource with multiple potential owners" do
      ...
      operator1 = AgentEntity.create_agent(
        "op1", 
        "Operator Alice",
        %{shift: "morning"},
      ...
      operator2 = AgentEntity.create_agent(
        "op2",
        "Operator Bob", 
        %{shift: "afternoon"},
      ...
      cnc_machine = AgentEntity.create_entity(
        "cnc1",
        "CNC Machine #1",
      ...
      |> Timeline.add_constraint("#{morning_shift.id}_end", "#{afternoon_shift.id}_start", {3600, 3600}) # 1 hour gap
  ...
  describe "STN solving with capability constraints" do
    test "PC-2 algorithm with agent capability dependencies" do
      ...
      architect = AgentEntity.create_agent(
        "arch1",
        "Senior Architect",
      ...
      engineer = AgentEntity.create_agent(
        "eng1", 
        "Structural Engineer",
      ...
      contractor = AgentEntity.create_agent(
        "cont1",
        "General Contractor",
        %{license: "commercial"},
      ...
      engineering_phase = Interval.new(
        DateTime.from_naive!(~N[2025-01-01 13:00:00], "Etc/UTC"),
        DateTime.from_naive!(~N[2025-01-01 16:00:00], "Etc/UTC"),
        agent: engineer,
        label: "Structural Analysis",
        metadata: %{required_capabilities: [:engineering_analysis], depends_on: [design_phase.id]}
      )
      
      construction_phase = Interval.new(
      ...
      |> Timeline.add_constraint("#{design_phase.id}_end", "#{engineering_phase.id}_start", {3600, 3600}) # 1 hour gap
      |> Timeline.add_constraint("#{engineering_phase.id}_end", "#{construction_phase.id}_start", {57600, 57600}) # 16 hour gap (overnight)
      ...
      design_to_construction = Timeline.get_constraint(
        solved_timeline,
      ...
      assert min_delay >= 3600 + 57600 # sum of explicit gaps between phases
    end
    ...
    test "temporal consistency with dynamic capability changes" do
      ...
      iot_device = AgentEntity.create_entity(
        "iot1",
        "Smart Sensor",
      ...
      updated_device = AgentEntity.add_capabilities(iot_device, [:communication, :data_transmission])
      communication_phase = Interval.new(
        DateTime.from_naive!(~N[2025-01-01 10:00:00], "Etc/UTC"),
        DateTime.from_naive!(~N[2025-01-01 14:00:00], "Etc/UTC"),
      ...
      ai_device = AgentEntity.add_capabilities(updated_device, [:decision_making, :autonomous_operation])
      autonomous_phase = Interval.new(
        DateTime.from_naive!(~N[2025-01-01 14:00:00], "Etc/UTC"),
      ...
      |> Timeline.add_constraint("#{sensing_phase.id}_end", "#{communication_phase.id}_start", {0, 0})
      |> Timeline.add_constraint("#{communication_phase.id}_end", "#{autonomous_phase.id}_start", {0, 0})
  ...
  describe "complex real-world scenarios" do
    test "hospital surgery scheduling with specialized capabilities" do
      ...
      cardiac_surgeon = AgentEntity.create_agent(
        "surgeon_cardiac",
        "Dr. Heart",
      ...
      anesthesiologist = AgentEntity.create_agent(
        "anesthesia1",
        "Dr. Sleep",
      ...
      surgical_nurse = AgentEntity.create_agent(
        "nurse_surgical",
        "Nurse Precision",
      ...
      operating_room = AgentEntity.create_entity(
        "or_1",
        "Operating Room 1",
      ...
      anesthesia_induction = Interval.new(
        DateTime.from_naive!(~N[2025-01-01 08:00:00], "Etc/UTC"),
      ...
      |> Timeline.add_constraint("#{anesthesia_induction.id}_end", "#{cardiac_surgery.id}_start", {0, 0})
    ...
    test "autonomous vehicle fleet coordination" do
      ...
      delivery_truck = AgentEntity.create_entity(
        "truck1",
        "Delivery Truck Alpha",
        %{capacity: 1000, fuel: 80, location: "depot"}
      )
      
      passenger_car = AgentEntity.create_entity(
        "car1", 
        "Autonomous Sedan",
      ...
      maintenance_truck = AgentEntity.remove_capabilities(
        autonomous_truck,
        [:autonomous_driving, :navigation, :decision_making]
      )
      
      truck_maintenance = Interval.new(
        DateTime.from_naive!(~N[2025-01-01 11:00:00], "Etc/UTC"),
        DateTime.from_naive!(~N[2025-01-01 12:00:00], "Etc/UTC"),
      ...
      |> Timeline.add_constraint("#{delivery_mission.id}_end", "#{truck_maintenance.id}_start", {0, 0})
