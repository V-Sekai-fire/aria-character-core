...
defmodule NodeLibrary.KHRInteractivity.Unit.MathNodesTest do
  ...
  describe "math arithmetic - binary operations" do
    test "khr_math_add with two positive numbers" do
      state = StateV2.new()
      result_state = KHRInteractivityDomain.math_add(state, [25, 2.1, 3.5])
    ...
    test "khr_math_add with negative numbers" do
      state = StateV2.new()
      result_state = KHRInteractivityDomain.math_add(state, [26, -1.5, -2.5])
    ...
    test "khr_math_sub basic subtraction" do
      state = StateV2.new()
      result_state = KHRInteractivityDomain.math_sub(state, [27, 8.7, 3.5])
    ...
    test "khr_math_div basic division" do
      state = StateV2.new()
      result_state = KHRInteractivityDomain.math_div(state, [29, 15.0, 3.0])
    ...
    test "khr_math_div by zero with negative numerator" do
      state = StateV2.new()
      result_state = KHRInteractivityDomain.math_div(state, [31, -5.0, 0.0])
    ...
    test "khr_math_rem with zero divisor" do
      state = StateV2.new()
      result_state = KHRInteractivityDomain.math_rem(state, [34, 7.0, 0.0])
    ...
    test "khr_math_min with two different numbers" do
      state = StateV2.new()
      result_state = KHRInteractivityDomain.math_min(state, [35, 2.1, 3.7])
    ...
    test "khr_math_max with two different numbers" do
      state = StateV2.new()
      result_state = KHRInteractivityDomain.math_max(state, [37, 2.1, 3.7])
    ...
    test "khr_math_mix basic interpolation" do
      state = StateV2.new()
      result_state = KHRInteractivityDomain.math_mix(state, [39, 1.0, 5.0, 0.5])
    ...
    test "khr_math_mix at endpoints" do
      state = StateV2.new()
      result_state_0 = KHRInteractivityDomain.math_mix(state, [40, 5.0, 10.0, 0.0])
      result_state_1 = KHRInteractivityDomain.math_mix(result_state_0, [41, 5.0, 10.0, 1.0])
      
      assert StateV2.get_fact(result_state_0, 40, "value") == 5.0
      assert StateV2.get_fact(result_state_1, 41, "value") == 10.0
    end
  end
  ...
  describe "math arithmetic - ternary operations" do
    test "khr_math_clamp within bounds" do
      state = StateV2.new()
      result_state = KHRInteractivityDomain.math_clamp(state, [42, 5.0, 3.0, 7.0])
    ...
    test "khr_math_clamp above maximum" do
      state = StateV2.new()
      result_state = KHRInteractivityDomain.math_clamp(state, [44, 9.0, 3.0, 6.0])
    ...
    test "khr_math_clamp with reversed bounds" do
      state = StateV2.new()
      result_state = KHRInteractivityDomain.math_clamp(state, [45, 5.0, 7.0, 3.0])
      
      # KHR spec: min(max(a, min(b, c)), max(b, c))
      # min(max(5.0, min(7.0, 3.0)), max(7.0, 3.0))
      # min(max(5.0, 3.0), 7.0)
      # min(5.0, 7.0)
      # 5.0
