      ...
      case AriaSerial.JsonStorage.generate_and_register_serial(year, week, factory, tool_code, file_info) do
        {:ok, serial} ->
          ...
          case add_serial_to_file(file_path, serial) do
            :ok ->
              Mix.shell().info("   ✅ Serial number added and registered in lookup")
            {:error, reason} ->
              Mix.shell().error("   ❌ File injection failed: #{reason}")
              Mix.shell().error("   ⚠️  Serial #{serial} was registered but not added to file")
          end
        {:error, reason} ->
          Mix.shell().error("   ❌ Failed to generate/register serial: #{inspect(reason)}")
      end
    end
