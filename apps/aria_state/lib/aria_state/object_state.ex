...
defmodule AriaState.ObjectState do
  ...
  |> AriaState.ObjectState.set_fact(42, :value, 3.14159)
  |> AriaState.ObjectState.set_fact(43, :operation, :add)

  # Atom predicates for performance
  |> AriaState.ObjectState.set_fact("npc1", :status, :active)
  |> AriaState.ObjectState.set_fact("npc1", :ai_state, {:planning, "attack_player"})

  # Mixed types work naturally
  AriaState.ObjectState.get_fact(state, 42, :value)  # => 3.14159
  AriaState.ObjectState.get_fact(state, "player", "location")  # => "room1"
  ```
  ...
  AriaState.ObjectState.exists?(state, &String.contains?(&1, "chair"), "status", "available")
  ```
  """
  ...
  AriaState.ObjectState.forall?(state, &String.contains?(&1, "door"), "status", "locked")
  ```
  """
  ...
  @doc """
  Converts from legacy RelationalState format to ObjectState format.

  This function helps with migration from the old {predicate, subject} format
  to the new entity-first {subject, predicate} format.
  """
  @spec from_relational_state(AriaState.RelationalState.t()) :: t()
  def from_relational_state(%AriaState.RelationalState{data: legacy_data}) do
    converted_data =
      legacy_data
  ...
  @doc """
  Converts to RelationalState format for internal AriaEngine operations.

  This function allows ObjectState to be used with internal systems
  that expect the predicate-first format for performance reasons.
  """
  @spec to_relational_state(t()) :: AriaState.RelationalState.t()
  def to_relational_state(%__MODULE__{data: data}) do
    converted_data =
      data
      |> Enum.map(fn {{subject, predicate}, fact_value} -> {{predicate, subject}, fact_value} end)
      |> Map.new()

    %AriaState.RelationalState{data: converted_data}
  end
end
