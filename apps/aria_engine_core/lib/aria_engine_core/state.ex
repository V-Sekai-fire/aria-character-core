...
defmodule AriaEngineCore.State do
  ...
  def new(data) when is_map(data) do
    AriaState.RelationalState.new(data)
  end
  ...
  def set_fact(state, predicate, subject, fact_value) do
    AriaState.RelationalState.set_fact(state, predicate, subject, fact_value)
  end

  defdelegate has_subject?(state, predicate, subject), to: AriaState.RelationalState
  defdelegate get_subjects_with_fact(state, predicate, fact_value), to: AriaState.RelationalState
  ...
  def has_subject?(state, predicate, subject) do
    AriaState.RelationalState.has_subject?(state, predicate, subject)
  end
  ...
  def has_subject_variable?(state, subject) do
    AriaState.RelationalState.has_subject_variable?(state, subject)
  end

  @doc "Gets a list of all subjects that have properties."
  @spec get_subjects(t()) :: [subject()]
  def get_subjects(state) do
    AriaState.RelationalState.get_subjects(state)
  end
  ...
  def get_subject_properties(state, subject) do
    AriaState.RelationalState.get_subject_properties(state, subject)
  end
  ...
  def matches?(state, predicate, subject, fact_value) do
    AriaState.RelationalState.matches?(state, predicate, subject, fact_value)
  end
  ...
  def exists?(state, predicate, fact_value, subject_filter \\ nil) do
    AriaState.RelationalState.exists?(state, predicate, fact_value, subject_filter)
  end
  ...
  def forall?(state, predicate, fact_value, subject_filter) when is_function(subject_filter, 1) do
    AriaState.RelationalState.forall?(state, predicate, fact_value, subject_filter)
  end
  ...
  def get_subjects_with_fact(state, predicate, fact_value) do
    AriaState.RelationalState.get_subjects_with_fact(state, predicate, fact_value)
  end
  ...
  def get_subjects_with_predicate(state, predicate) do
    AriaState.RelationalState.get_subjects_with_predicate(state, predicate)
  end
