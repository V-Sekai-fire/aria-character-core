...
defmodule AriaHybridPlanner.PlanCore do
  ...
  def ipyhop(domain, state, solution_tree, opts), do: Plan.Core.ipyhop(domain, state, solution_tree, opts)

  # Delegate execution functions to Plan.Execution
  @spec run_lazy_refineahead(AriaEngine.Domain.Core.t(), AriaEngine.State.t(), term(), keyword()) :: {:ok, AriaEngine.State.t()} | {:error, String.t()}
  def run_lazy_refineahead(domain, state, plan, opts \\ []), do: Plan.Execution.run_lazy_refineahead(domain, state, plan, opts)

  # Delegate replanning functions to Plan.Backtracking
  @spec replan(AriaEngine.Domain.Core.t(), AriaEngine.State.t(), solution_tree(), node_id(), keyword()) :: plan_result()
  def replan(domain, state, solution_tree, fail_node_id, opts \\ []), do: Plan.Backtracking.replan(domain, state, solution_tree, fail_node_id, opts)
end
