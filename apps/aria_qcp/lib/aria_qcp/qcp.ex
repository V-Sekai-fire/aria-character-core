...
defmodule AriaQcp.QCP do
  ...
  ## Examples
      ...
      iex> {:ok, {rotation, _translation}} = AriaQcp.QCP.weighted_superpose(moved, target, [], false)
      iex> {_, _, _, w} = rotation
      iex> abs(w - 0.7071067811865476) < 1.0e-10
      true
  ...
  defp calculate_single_point_rotation(moved_point, target_point) do
    ...
    if u_length < 1.0e-15 or v_length < 1.0e-15 do
      ...
      cond do
        ...
        true ->
          # Use the half-angle formula for robust quaternion calculation
          # This matches the expected test values

          cross = Vector3.cross(u_norm, v_norm)
          cross_length = Vector3.length(cross)

          if cross_length < 1.0e-15 do
            # Vectors are parallel, return identity
            {:ok, {0.0, 0.0, 0.0, 1.0}}
          else
            # Calculate the half-angle quaternion components
            # cos(θ/2) = sqrt((1 + cos(θ)) / 2) = sqrt((1 + dot) / 2)
            # sin(θ/2) = sqrt((1 - cos(θ)) / 2) = sqrt((1 - dot) / 2)

            w = :math.sqrt((1.0 + dot) / 2.0)
            sin_half_angle = :math.sqrt((1.0 - dot) / 2.0)

            # Normalize cross product to get rotation axis
            {axis_x, axis_y, axis_z} = Vector3.scale(cross, 1.0 / cross_length)

            # Scale axis by sin(θ/2)
            x = axis_x * sin_half_angle
            y = axis_y * sin_half_angle
            z = axis_z * sin_half_angle
