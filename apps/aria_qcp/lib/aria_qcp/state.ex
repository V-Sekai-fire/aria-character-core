...
defmodule AriaQcp.State do
  @moduledoc """
  State management for QCP algorithm.

  This module handles initialization of QCP state, centering of points,
  weight normalization, and calculation of the inner product matrix.
  """
  ...
  @type qcp_state :: %{
    ...
    translate: boolean(),
    precision: float(),
    moved_center: Vector3.t(),
    target_center: Vector3.t(),
    w_sum: float(),
    # Inner product matrix components
    sum_xx: float(), sum_xy: float(), sum_xz: float(),
    sum_yx: float(), sum_yy: float(), sum_yz: float(),
    sum_zx: float(), sum_zy: float(), sum_zz: float(),
    # Derived sums for characteristic polynomial
    sum_xx_plus_yy: float(), sum_xx_minus_yy: float(),
    sum_xy_plus_yx: float(), sum_xy_minus_yx: float(),
    sum_xz_plus_zx: float(), sum_xz_minus_zx: float(),
    sum_yz_plus_zy: float(), sum_yz_minus_zy: float(),
    max_eigenvalue: float(),
    # Robustness tracking
    numerical_warnings: [term()]
  }

  @doc """
  Initialize QCP state with point sets, weights, and configuration.
  """
  @spec initialize_qcp_state(point_set(), point_set(), weights(), boolean(), float()) ::
          {:ok, qcp_state()} | {:error, term()}
  ...
  @doc """
  Calculate the inner product matrix from centered points.
  """
  @spec calculate_inner_product(qcp_state()) :: {:ok, qcp_state()} | {:error, term()}
  ...
  @spec center_and_translate_points(qcp_state()) :: qcp_state()
