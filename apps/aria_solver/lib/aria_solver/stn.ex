...
defmodule AriaSolver.STN do
  @moduledoc """
  Matrix-based Simple Temporal Network (STN) solver for AriaSolver.

  This module provides mathematically correct STN constraint solving using
  a distance matrix representation with MiniZinc constraint satisfaction.
  Migrated from `aria_minizinc_stn` as part of ADR-193 layered architecture consolidation.
  ...
  ## Usage
      ...
      {:ok, result} = AriaSolver.STN.solve_stn(stn)
  ...
  defp template_path do
    Path.join([Application.app_dir(:aria_solver), "priv", "templates", "stn_temporal.mzn.eex"])
  end
end
