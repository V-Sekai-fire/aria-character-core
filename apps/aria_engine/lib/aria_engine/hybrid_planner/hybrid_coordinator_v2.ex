...
defmodule AriaEngine.HybridPlanner.HybridCoordinatorV2 do
  ...
  def new(strategies, opts \\ []) do
    ...
    case Strategies.validate_strategy_compatibility(strategies) do
      :ok -> :ok
  ...
  def get_strategy_info(%__MODULE__{} = coordinator) do
    %{
      planning_strategy: coordinator.planning_strategy.strategy_info(),
      temporal_strategy: coordinator.temporal_strategy.strategy_info(),
      state_strategy: coordinator.state_strategy.strategy_info(),
      domain_strategy: coordinator.domain_strategy.strategy_info(),
      logging_strategy: coordinator.logging_strategy.strategy_info(),
      execution_strategy: coordinator.execution_strategy.strategy_info(),
  ...
  defp get_strategy_composition_info(strategies) do
    ...
    |> Enum.map(fn {strategy_type, strategy_module} ->
      {strategy_type, %{
        ...
        info: strategy_module.strategy_info()
      }}
    end)
    |> Enum.into(%{})
  end
