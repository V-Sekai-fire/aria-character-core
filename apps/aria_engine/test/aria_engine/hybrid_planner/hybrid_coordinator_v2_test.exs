...
defmodule AriaEngine.HybridPlanner.HybridCoordinatorV2Test do
  ...
  describe "basic coordinator functionality" do
    test "can create coordinator with default strategies" do
      ...
      assert coordinator.strategies.planning_strategy != nil
      assert coordinator.strategies.temporal_strategy != nil
      assert coordinator.strategies.state_strategy != nil
      assert coordinator.strategies.domain_strategy != nil
      assert coordinator.strategies.logging_strategy != nil
      assert coordinator.strategies.execution_strategy != nil
    end
    ...
    test "coordinator maintains metadata and configuration" do
      ...
      assert Map.has_key?(coordinator.metadata, :coordinator_id)
    end
  end
  ...
  describe "strategy injection and composition" do
    test "can inject custom mock strategy" do
      ...
      mock_planning = MockPlanningStrategy.new(
  ...
  describe "functional equivalence with original planner" do
    setup do
      ...
      domain = Domain.Core.new()
      |> Domain.Core.add_action(:test_action, fn state, [arg] ->
        {:ok, StateV2.set_fact(state, "result", "value", arg)}
      end)
      |> Domain.Core.add_task_methods("test_task", [
  ...
  describe "strategy composition and profiling" do
    ...
    test "strategies can track their own call counts and timing" do
      mock_planning = MockPlanningStrategy.new(call_delay: 10)
  ...
  describe "backward compatibility" do
    test "coordinator provides same interface as original planner" do
      ...
      assert function_exported?(HybridCoordinatorV2, :execute_plan, 2)
      assert function_exported?(HybridCoordinatorV2, :validate_plan, 2)
    end
