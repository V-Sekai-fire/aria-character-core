...
defmodule AriaEngine.Timeline.STNLODTest do
  ...
  describe "LOD and Unit System" do
    ...
    test "automatic rescaling with constraints" do
      ...
      stn = stn 
        ...
        |> STN.add_constraint("t1", "t2", {1000, 2000})  # 1-2 seconds
      
      # Convert to seconds
      second_stn = STN.convert_units(stn, :second)
      constraint = STN.get_constraint(second_stn, "t1", "t2")
      
      assert constraint == {1, 2}  # Should be converted to seconds
    end
  end
  ...
  describe "Boolean-like Operators" do
    ...
    test "OR operation with relaxed constraints" do
      ...
      assert constraint == {5, 30}  # Min of mins, max of maxes
    end
  ...
  describe "Auto-rescaling Compatibility" do
    test "union auto-rescales incompatible units" do
      stn1 = STN.new(time_unit: :millisecond, auto_rescale: true)
        |> STN.add_time_point("t1")
        |> STN.add_constraint("t1", "t1", {0, 0})
      
      stn2 = STN.new(time_unit: :second, auto_rescale: true)
        |> STN.add_time_point("t2")
        |> STN.add_constraint("t2", "t2", {0, 0})
      ...
      assert STN.consistent?(result)
      assert result.time_unit == :millisecond  # Uses first STN's units
    end

    test "union auto-rescales incompatible LOD levels" do
      stn1 = STN.new(lod_level: :high, auto_rescale: true)
        |> STN.add_time_point("t1")
      
      stn2 = STN.new(lod_level: :low, auto_rescale: true)
        |> STN.add_time_point("t2")
      
      result = STN.union(stn1, stn2)
      
      assert STN.consistent?(result)
      assert result.lod_level == :high  # Uses first STN's LOD
    end
  ...
  describe "Constant Work Pattern Support" do
    ...
    test "constant work STN remains consistent" do
      stn = STN.new(constant_work_enabled: true, max_timepoints: 16)
  ...
  describe "Performance and Integration" do
    test "parallel solving with LOD system" do
      ...
      stn_with_intervals = STN.from_datetime_intervals(intervals, time_unit: :second, lod_level: :high)
    ...
    test "complex boolean operations with LOD" do
      ...
      stn_high = STN.new(lod_level: :high, time_unit: :millisecond)
        |> STN.add_time_point("high_res")
      
      stn_low = STN.new(lod_level: :low, time_unit: :millisecond)
