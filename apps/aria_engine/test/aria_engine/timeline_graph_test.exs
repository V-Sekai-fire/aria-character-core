...
defmodule AriaEngine.TimelineGraphTest do
  ...
  describe "new/0" do
    test "creates empty timeline graph with StateV2" do
      ...
      assert timeline_graph.state == %StateV2{}
  ...
  describe "create_entity/4" do
    ...
    test "creates entity with automatic timeline attachment", %{timeline_graph: timeline_graph} do
      ...
      {:ok, updated_graph, entity_id} = TimelineGraph.create_entity(
        timeline_graph,
        "chair1",
        "Wooden Chair",
        properties
      ...
      assert lod in [:very_low, :low, :medium]  # Depends on initial determination
  ...
  describe "add_capabilities/3" do
    setup do
      ...
      {:ok, timeline_graph, _} = TimelineGraph.create_entity(
        timeline_graph,
        "door1",
        "Castle Door",
    ...
    test "transitions entity to agent when adding action capabilities", %{timeline_graph: timeline_graph} do
    ...
    test "handles non-existent entity", %{timeline_graph: timeline_graph} do
      result = TimelineGraph.add_capabilities(
        timeline_graph,
        "nonexistent",
        [:some_capability]
      )
    ...
    test "adds capabilities without agent transition", %{timeline_graph: timeline_graph} do
      ...
      {:ok, updated_graph} = TimelineGraph.add_capabilities(
        timeline_graph,
        "door1",
        [:data_storage]  # Assuming this doesn't qualify as action capability
      )
  ...
  describe "entity property management" do
    ...
    test "set_entity_property/4 updates state and triggers timeline growth", %{timeline_graph: timeline_graph} do
      initial_props = TimelineGraph.get_entity_properties(timeline_graph, "player")
      assert initial_props["location"] == "room1"
      
      {:ok, updated_graph} = TimelineGraph.set_entity_property(
        timeline_graph,
        "player",
        "location",
        "room2"
      ...
      assert updated_props["health"] == 100  # Other properties unchanged
    ...
    test "get_entity_properties/2 uses entity-first StateV2 API", %{timeline_graph: timeline_graph} do
    ...
    test "set_entity_property/4 handles non-existent entity", %{timeline_graph: timeline_graph} do
      result = TimelineGraph.set_entity_property(
        timeline_graph,
        "nonexistent",
        "some_prop",
        "some_value"
  ...
  describe "entity and agent queries" do
    ...
    test "get_agent_ids/1 returns only agents", %{timeline_graph: timeline_graph} do
      ...
      assert "chair1" not in agent_ids  # Entity, not agent
      assert "table1" not in agent_ids  # Entity, not agent
  ...
  describe "LOD management" do
    ...
    test "process_lod_promotions/1 promotes queued entities", %{timeline_graph: timeline_graph} do
      ...
      timeline_graph_with_queue = %{timeline_graph | 
        lod_promotion_queue: ["npc1"]
      }
