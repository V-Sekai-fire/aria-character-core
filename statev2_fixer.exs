...
# State to State Migration Fixer Tool
# Comprehensive tool for migrating State references to State throughout codebase
# Based on successful migration patterns from migrate_statev2.exs

defmodule StateV2Fixer do
  @moduledoc """
  Comprehensive State to State migration tool.

  Features:
  - Automatic detection of all State references
  ...
  defp transformations do
    [
      ...
      {:function_call, ~r/State\.update_fact\(([^,]+),\s*([^,]+),\s*([^,]+),\s*([^)]+)\)/,
       "State.set_fact(\\1, \\3, \\2, \\4)", "update_fact → set_fact with parameter reorder"},

      {:function_call, ~r/State\.matches_exactly\?\(([^,]+),\s*([^,]+),\s*([^,]+),\s*([^)]+)\)/,
       "State.matches?(\\1, \\3, \\2, \\4)", "matches_exactly? → matches? with parameter reorder"},

      # Simple function name changes
      {:function_name, ~r/State\.matches_exactly\?/, "State.matches?", "matches_exactly? → matches?"},
      {:function_name, ~r/State\.get_fact/, "State.get_fact", "get_fact function"},
      {:function_name, ~r/State\.set_fact/, "State.set_fact", "set_fact function"},
      {:function_name, ~r/State\.new/, "State.new", "new function"},
      {:function_name, ~r/State\.has_predicate\?/, "State.has_predicate?", "has_predicate? function"},

      # Struct patterns
      {:struct_pattern, ~r/%State\{/, "%State{", "struct pattern"},

      # Module references in aliases and imports
      {:alias_import, ~r/alias\s+AriaEngine\.State/, "alias State", "alias statement"},
      {:alias_import, ~r/alias\s+State/, "alias State", "alias statement"},
      {:alias_import, ~r/import\s+AriaEngine\.State/, "import State", "import statement"},
      {:alias_import, ~r/import\s+State/, "import State", "import statement"},

      # Standalone module references (be careful not to match in comments/strings)
      {:module_ref, ~r/\bStateV2\b(?!\w)/, "State", "module reference"},

      # Type annotations
      {:type_annotation, ~r/State\.t\(\)/, "State.t()", "type annotation"},
      {:type_annotation, ~r/@type.*State/, "@type State", "type definition"},
    ]
  end
  ...
  def scan_codebase(directory \\ ".") do
    IO.puts("🔍 Scanning codebase for State references...")
    ...
    if length(files_with_issues) == 0 do
      IO.puts("✅ No State references found!")
    else
      IO.puts("\n📋 Found State references in #{length(files_with_issues)} files:")
      ...
      IO.puts("\n📊 Total: #{total_issues} State references across #{length(files_with_issues)} files")
    ...
    case File.read(file_path) do
      {:ok, content} ->
        ...
        if length(changes) == 0 do
          IO.puts("✅ No State references found in this file.")
        else
  ...
  def apply_fixes(files, opts \\ []) do
    ...
    if dry_run do
      ...
      IO.puts("🔧 Applying State to State fixes...")
    end
  ...
  defp print_help do
    ...
    Commands:
      scan [directory]           Scan for State references (default: current directory)
      preview <file>             Preview changes for a specific file
      fix --all                  Fix all files with State references
    ...
    Features:
      - Automatic State → State migration
