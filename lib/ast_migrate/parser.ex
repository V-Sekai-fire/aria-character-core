...
defmodule AstMigrate.Parser do
  @moduledoc """
  Advanced AST parsing using Sourceror for robust code transformation.
  ...
  @doc """
  Parse Elixir source code into a Sourceror zipper for advanced manipulation.
  ...
  @doc """
  Parse Elixir source code string into a Sourceror zipper.
  """
    ...
    try do
      case Sourceror.parse_string(content, options) do
        ...
        {:error, reason} ->
          ...
          # Fallback to basic parsing if Sourceror fails
  ...
  @doc """
  Convert a Sourceror zipper back to formatted source code.
  ...
  def to_string(zipper, opts \\ []) do
    ...
    try do
      ...
      code = if options[:format_code] do
        ast
        |> Sourceror.to_string()
        |> Code.format_string!()
        |> IO.iodata_to_binary()
  ...
  @doc """
  Apply a transformation function to a zipper while preserving structure.

  The transformation function receives a zipper and should return a modified zipper.
  """
  @spec transform(Sourceror.Zipper.t(), (Sourceror.Zipper.t() -> Sourceror.Zipper.t())) :: Sourceror.Zipper.t()
  def transform(zipper, transform_fn) when is_function(transform_fn, 1) do
    ...
    try do
      ...
      exception ->
        ...
        # Return original zipper if transformation fails
        zipper
    end
  end

  @doc """
  Find all nodes in the zipper that match a given pattern.
  ...
  def find_nodes(zipper, matcher_fn) when is_function(matcher_fn, 1) do
    Logger.debug("Finding nodes in zipper",
      module: :ast_migrate_parser,
      operation: :find_nodes
    )

    results = Sourceror.Zipper.traverse(zipper, [], fn zipper, acc ->
  ...
  @doc """
  Replace all nodes that match a pattern with the result of a transformation function.
  """
  @spec replace_nodes(Sourceror.Zipper.t(), (term() -> boolean()), (Sourceror.Zipper.t() -> term())) :: Sourceror.Zipper.t()
  def replace_nodes(zipper, matcher_fn, replacer_fn) when is_function(matcher_fn, 1) and is_function(replacer_fn, 1) do
    ...
    {result_zipper, final_count} = Sourceror.Zipper.traverse(zipper, replacement_count, fn zipper, count ->
      node = Sourceror.Zipper.node(zipper)
      if matcher_fn.(node) do
