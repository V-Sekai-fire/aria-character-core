defmodule Mix.Tasks.Ast.Commit do
      ...
      try do
        with {:ok, config} <- parse_configuration(opts),
             ...
             {:ok, successful_files, failed_files} <-
               apply_transformations(target_files, rule_modules, config) do
        ...
        exception ->
          ...
          Logger.error("Exception in ast.commit task",
  ...
  defp parse_files(files_string) do
    files_string |> String.split(",") |> Enum.map(&String.trim/1) |> Enum.filter(&(&1 != ""))
  end
  ...
  defp rule_name_to_module(rule_name) do
    module_name = rule_name |> Macro.camelize() |> String.replace_suffix("", "")
  ...
  defp discover_files_by_pattern(pattern) do
    ...
    patterns |> Enum.flat_map(&Path.wildcard/1) |> Enum.uniq() |> Enum.filter(&File.regular?/1)
  end
  ...
  defp print_results(successful_files, failed_files, config) do
    ...
    success_rate =
      if total_files > 0 do
        length(successful_files) / total_files * 100
      else
        0
      end
    ...
    if length(failed_files) > 0 do
      ...
      Enum.each(failed_files, fn file -> Mix.shell().info("  - #{file}") end)
    end

    if config.verbose and length(successful_files) > 0 do
      Mix.shell().info("")
      Mix.shell().info("✅ Successful files:")
      Enum.each(successful_files, fn file -> Mix.shell().info("  - #{file}") end)
    end
  end

  defp print_help do
    Mix.shell().info("mix ast.commit - Phase 1 AST Migration Tool
