...
defmodule AriaEngine.MCP.SSEServer do
  ...
  post "/tools" do
    case conn.body_params do
      %{"method" => "tools/call", "params" => %{"name" => tool_name, "arguments" => args}} ->
        handle_tool_call(conn, tool_name, args)
  ...
  defp handle_tool_call(conn, tool_name, args) do
    Logger.info("Received #{tool_name} tool call via HTTP")
    
    try do
      # Use the extensible tool system
