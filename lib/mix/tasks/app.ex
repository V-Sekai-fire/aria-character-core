...
defmodule Mix.Tasks.App do
  ...
  def run(args) do
    case args do
      ...
      _ ->
        Mix.raise(
          "Invalid arguments. Use mix app.start, mix app.stop, mix app.status, or mix app.logs"
        )
    end
  end
  ...
  defp start_app() do
    ...
    if File.exists?(pid_file) and
         System.cmd("pgrep", ["-F", pid_file]) |> elem(0) |> String.trim() != "" do
  ...
  defp stop_app() do
    ...
    if File.exists?(pid_file) do
      ...
      if System.cmd("pgrep", ["-F", pid]) |> elem(0) |> String.trim() != "" do
        ...
        Mix.shell().info(
          "⚠️  Elixir application process not found, PID file exists. Cleaning up PID file."
        )
      end
      ...
      Mix.shell().info(
        "⚠️  Elixir application PID file not found. Attempting to kill any running phoenix.server processes..."
      )
  ...
  defp status_app() do
    ...
    if File.exists?(pid_file) do
      ...
      if System.cmd("pgrep", ["-F", pid]) |> elem(0) |> String.trim() != "" do
        ...
        Mix.shell().info(
          "❌ Elixir application is STOPPED (PID file found but process not running). Cleaning up PID file."
        )
