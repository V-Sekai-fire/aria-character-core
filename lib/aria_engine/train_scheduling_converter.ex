...
defmodule AriaEngine.TrainSchedulingConverter do
  ...
  defp create_train_service_activities() do
    ...
    route1_services = [
      {"R1a", 0},   # Start at time 0
      {"R1b", 30},  # Start at time 30
      {"R1c", 60},  # Start at time 60
      {"R1d", 90},  # Start at time 90
      {"R1e", 120}, # Start at time 120
      {"R1f", 150}  # Start at time 150
    ]

    # Route 2: F -> E -> D -> C -> B -> A (services R2a through R2f)
    route2_services = [
      {"R2a", 0},   # Start at time 0
      {"R2b", 30},  # Start at time 30
      {"R2c", 60},  # Start at time 60
      {"R2d", 90},  # Start at time 90
      {"R2e", 120}, # Start at time 120
      {"R2f", 150}  # Start at time 150
    ]

    # Create activities for route 1 services
    route1_activities = Enum.flat_map(route1_services, fn {service_id, start_time} ->
      create_route1_activities(service_id, start_time)
    end)

    # Create activities for route 2 services
    route2_activities = Enum.flat_map(route2_services, fn {service_id, start_time} ->
  ...
  defp create_route1_activities(service_id, service_start_time) do
    ...
    {activities, _} = Enum.reduce(stations, {[], service_start_time}, fn station, {acc_activities, current_time} ->
      ...
      activity = %{
        ...
        "participants" => [], # Will be assigned by scheduler
        "type" => "station_visit",
        "metadata" => %{
      ...
      next_time = if station_index < length(stations) - 1 do
        travel_time = Enum.at(travel_times, station_index)
        current_time + 2 + travel_time # 2 min station wait + travel time
      else
        current_time + 2 # Final station, just wait time
      end
  ...
  defp create_station_visit_duration(start_time, _station, _service_id) do
    ...
    end_datetime = DateTime.add(start_datetime, 2 * 60, :second) # 2 minute station visit
  ...
  defp create_station_resources() do
    %{
      "platform_A" => %{
        "type" => "train_platform",
        "capacity" => 2, # Station A has 2 platforms
        "current_usage" => 0,
        "constraints" => %{
          "min_separation_time" => 4, # 4 minutes minimum separation
      ...
      "platform_B" => %{
        "type" => "train_platform",
        "capacity" => 1, # Station B has 1 platform
        "current_usage" => 0,
        "constraints" => %{
      ...
      "platform_C" => %{
        "type" => "train_platform",
        "capacity" => 1, # Station C has 1 platform
        "current_usage" => 0,
        "constraints" => %{
          "min_separation_time" => 4,
          "max_concurrent_trains" => 1
      ...
      "platform_D" => %{
        "type" => "train_platform",
        "capacity" => 1, # Station D has 1 platform
        "current_usage" => 0,
        "constraints" => %{
          "min_separation_time" => 4,
          "max_concurrent_trains" => 1
      ...
      "platform_E" => %{
        "type" => "train_platform",
        "capacity" => 1, # Station E has 1 platform
        "current_usage" => 0,
        "constraints" => %{
          "min_separation_time" => 4,
          "max_concurrent_trains" => 1
      ...
      "platform_F" => %{
        "type" => "train_platform",
        "capacity" => 2, # Station F has 2 platforms
        "current_usage" => 0,
        "constraints" => %{
          "min_separation_time" => 4,
          "max_concurrent_trains" => 2
  ...
  defp create_scheduling_constraints() do
    %{
      "max_concurrent_activities" => 12, # All 12 services can run concurrently
      "require_resources" => true,
      "makespan_limit" => 240, # 240 minutes total time
      "min_separation_time" => 4, # 4 minutes minimum separation
