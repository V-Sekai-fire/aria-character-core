...
defmodule AriaEngine.HybridPlanner.PlanTransformer do
  ...
  def convert_to_planning_params(mcp_request) do
    ...
    Logger.info("🔧 PlanTransformer: #{length(activities)} activities, #{length(entities)} entities, #{length(resources)} resources")
  ...
  """
  @spec transform_activities_to_domain(list(), list(), list(), map()) :: {:ok, map()} | {:error, String.t()}
