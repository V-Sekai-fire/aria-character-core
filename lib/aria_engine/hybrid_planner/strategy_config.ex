...
defmodule HybridPlanner.StrategyConfig do
  ...
  """
  @spec load_coordinator_from_config(keyword()) :: {:ok, HybridCoordinatorV2.t()} | {:error, String.t()}
  def load_coordinator_from_config(opts \\ []) do
    ...
    with {:ok, config} <- load_config_from_application(profile),
         ...
         {:ok, coordinator} <- StrategyFactory.create_coordinator(factory, config, coordinator_opts) do
      {:ok, coordinator}
    else
  ...
  """
  @spec load_coordinator_from_env(keyword()) :: {:ok, HybridCoordinatorV2.t()} | {:error, String.t()}
  ...
  """
  @spec load_config_from_application(profile_name()) :: {:ok, strategy_config()} | {:error, String.t()}
  def load_config_from_application(profile \\ :default) do
    try do
      # Load base configuration
      base_config = Application.get_env(:aria_engine, :hybrid_planner, %{})
      |> Map.get(:default_strategy_config, @default_config)

      # Apply profile overrides if requested
      final_config = if profile != :default do
        profile_overrides = Application.get_env(:aria_engine, :hybrid_planner, %{})
  ...
  def load_config_from_environment() do
    try do
      ...
      env_overrides = Enum.reduce(@env_mappings, %{}, fn {strategy_type, env_var}, acc ->
  ...
  def validate_config(config) when is_map(config) do
    ...
    missing_strategies = Enum.filter(required_strategies, fn strategy ->
      not Map.has_key?(config, strategy)
    end)

    invalid_values = Enum.filter(config, fn {_key, value} ->
      not is_atom(value)
    end)
  ...
  def get_profile_config(profile_name) do
    profiles = Application.get_env(:aria_engine, :hybrid_planner, %{})
    |> Map.get(:strategy_profiles, %{})
  ...
  def create_adaptive_config(opts \\ []) do
    ...
    perf_config = case performance_level do
    ...
    debug_config = case debug_level do
