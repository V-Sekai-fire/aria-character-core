...
defmodule Timeline.Internal.STN.PC2 do
    ...
    case check_direct_cycle_consistency(time_points, initial_constraints) do
      ...
      :consistent ->
        ...
        {final_constraints, consistent} =
          Enum.reduce_while(time_points, {initial_constraints, true}, fn k,
                                                                         {acc_constraints,
                                                                          acc_consistent} ->
        ...
        final_consistent =
          Enum.all?(Map.values(final_constraints), fn {min, max} -> min <= max end)

        # NEW: Check for self-loop consistency (d(i,i) must be {0,0})
        self_loop_consistent =
          Enum.all?(time_points, fn tp ->
  ...
  defp check_direct_cycle_consistency(time_points, constraints) do
    ...
    Enum.reduce_while(time_points, :consistent, fn i, _acc ->
      result =
        Enum.reduce_while(time_points, :consistent, fn j, _inner_acc ->
          if i == j do
            {:cont, :consistent}
          else
