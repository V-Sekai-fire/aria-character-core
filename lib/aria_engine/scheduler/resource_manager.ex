...
defmodule AriaEngine.Scheduler.ResourceManager do
  ...
  """
  def allocate_resources_for_activity(state, activity_id, required_capabilities, required_resources, entities, resources) do
    # Find available entity with required capabilities
    available_entity = find_available_entity_with_capabilities(state, required_capabilities, entities)
    
    # Check resource availability
    resources_available = Enum.all?(required_resources, fn resource_id ->
  ...
  def release_resources_for_activity(state, activity_id, required_resources, entities) do
    ...
    assigned_entity = if Enum.empty?(entities) do
      nil
    else
    ...
    # Release entity
    updated_state = if assigned_entity do
      state
  ...
  """
  def check_resource_availability(state, required_capabilities, required_resources, entities, resources) do
    # Check if there's an available entity with required capabilities
    entity_available = if Enum.empty?(required_capabilities) do
      true
    else
  ...
  def get_resource_utilization_snapshot(state, resources) do
    ...
    |> Enum.map(fn resource ->
      ...
      utilization_percentage = if resource.capacity > 0 do
