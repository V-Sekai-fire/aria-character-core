...
defmodule AriaEngine.Membrane.PipelineManager do
  ...
  @doc """
  Creates a new testing pipeline with the specified topology.
  """
  @spec create_testing_pipeline(topology()) :: {:ok, pipeline_id()} | {:error, term()}
  def create_testing_pipeline(topology) do
    Logger.warning("PipelineManager.create_testing_pipeline/1 not implemented for topology: #{topology}")
    {:error, :not_implemented}
  end

  @doc """
  Stops an existing pipeline.
  """
  @spec stop_pipeline(pipeline_id()) :: :ok | {:error, term()}
  def stop_pipeline(pipeline_pid) do
    Logger.warning("PipelineManager.stop_pipeline/1 not implemented for pid: #{inspect(pipeline_pid)}")
    {:error, :not_implemented}
  end
  ...
  @doc """
  Sends a request to a specific pipeline.
  """
  @spec send_request_to_pipeline(pipeline_id(), map()) :: :ok | {:error, term()}
  def send_request_to_pipeline(pipeline_pid, request_params) do
    Logger.warning("PipelineManager.send_request_to_pipeline/2 not implemented for pid: #{inspect(pipeline_pid)}, params: #{inspect(request_params)}")
    {:error, :not_implemented}
  end
